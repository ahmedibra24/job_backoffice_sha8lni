<x-app-layout>
    ...
    <div class="overflow-x-auto p-6">
        <div class="max-w-2xl mx-auto bg-white p-6 rounded-lg shadow">
            <form method="POST" action="{{ route('job-vacancy.store') }}" autocomplete="off">
                @csrf
                <div class="flex flex-row gap-6 justify-center">
                    ...
                    <div class=" w-full bg-slate-50 p-6 rounded-lg shadow mb-6">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" value="{{ old('title') }}" name="title" id="title" placeholder="Enter job title" class=" {{ $errors->has('title') ? 'border-red-500' : '' }} w-full border border-gray-300 p-2 rounded placeholder-gray-400" >
                            @error('title')
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" value="{{ old('location') }}" name="location" id="location" placeholder="Enter company location" class=" {{ $errors->has('location') ? 'border-red-500' : '' }} w-full border border-gray-300 p-2 rounded placeholder-gray-400" >
                            @error('location')
                                <div class="text-red-500 text-sm my-2">{{ $message }}</div>
                            @enderror   
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="number" value="{{ old('salary') }}" name="salary" id="salary" placeholder="Enter expected salary" class=" {{ $errors->has('salary') ? 'border-red-500' : '' }} w-full border border-gray-300 p-2 rounded placeholder-gray-400" >
                            @error('salary')
                                <div class="text-red-500 text-sm my-2">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        {{--! companies options --}}
                        {{--? show company select only for admin --}}
